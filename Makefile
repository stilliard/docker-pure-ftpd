...
test-tls: build run-tls logs-for-60 setup-bob test-bob
...
run-tls: kill
	sudo docker run -d --name ftpd_server -p 21:21 -p 30000-30009:30000-30009 -e "PUBLICHOST=localhost" -e "ADDED_FLAGS=-d -d --tls 2" -e "TLS_CN=localhost" -e "TLS_ORG=Demo" -e "TLS_C=UK" pure-ftp-demo
...
test-bob:
	...
	ls -alh" | lftp -u bob,test -e "set ssl:verify-certificate no" localhost 21
